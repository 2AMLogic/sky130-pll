...
* tb_divider_n4.sch -- standalone divider_intN PVT testbench, N=4 (issue #129)
*
...
* comment for the full rationale. This testbench straps N=4
* (NSEL[5:0]=000011), the DRAFT modulus range's floor (spec/target-spec.md
* row 4, N = 4-64) and the smallest modulus that takes the divider's
* *even* decode path (ODD=0, no prescaler period is ever stretched to /3;
* design/divider/DESIGN.md's "Modulus correctness across the DRAFT range"
* table). CLK is fixed at 250 MHz -- the design's own 250 MHz lock target,
* not a top-frequency stress point -- so this record exercises the modulus
* decode itself, not the timing margin sim/divider's own N=25 record
* already covers.
*
...
"
spice_ignore=false}
C {devices/title.sym} -200 500 0 0 {name=l4 author="2AM Logic (issue #129, sky130 standalone divider_intN PVT testbench, N=4)"}
