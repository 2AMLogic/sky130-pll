...
- **`<experiment-slug>`** — short, descriptive, kebab-case name for what is
  ...
  | `divider-n4` | sibling of `divider`, same DUT and open-loop method, straps N=4 (`NSEL[5:0]`=`000011`) at 250 MHz — the DRAFT modulus range's floor (spec/target-spec.md row 4) and the smallest modulus on the divider's even decode path. **Testbench only** — issue #129 added the manifest/schematic but did not run it (host compute contention); running it and minting the first record is issue #131. | #129 |
  | `divider-n64` | sibling of `divider`, same DUT and open-loop method, straps N=64 (`NSEL[5:0]`=`111111`) at 250 MHz — the DRAFT modulus range's ceiling and the largest modulus on the even decode path; N=64's long output period makes a full 45-point grid impractical in one pass (see the manifest's own `methodology_note`), so this campaign is landing one process-corner row at a time. **Partially run and recorded**: `sim/divider-n64/records/20260911-074438-073b241.md` (9/9 PASS — the full `tt` process-corner row, all 3 temps x 3 supplies; `ff`/`ss`/`sf`/`fs` rows remain, tracked by issue #130). | #129, #130 |
  | `divider-n5` | sibling of `divider`, same DUT and open-loop method, straps N=5 (`NSEL[5:0]`=`000100`) at 250 MHz — the smallest *odd* modulus in the DRAFT range, exercising the divider's odd (one-/3-period) decode path the N=4/N=64 records do not; meant to cover issue #129's two named corners (tt/27 °C/1.80 V, ss/125 °C/1.62 V) rather than the full grid. **Testbench only** — not yet run; issue #131. | #129 |
  | `divider-n63` | sibling of `divider`, same DUT and open-loop method, straps N=63 (`NSEL[5:0]`=`111110`) at 250 MHz — the largest odd modulus in the DRAFT range; like `divider-n64`, its long output period makes a full grid impractical, so it is meant to cover issue #129's two named corners only (issue #130 tracks widening it alongside `divider-n64`). **Testbench only** — not yet run; issue #131. | #129 |
