...
- **`analysis/`** is optional and, like `testbench/`, **mutable**: stdlib-only
  ...
  **two** campaigns' records: it restates this campaign's measured period jitter
  ...
  mode. The sixth
  (`sim/pll-lock-mc/analysis/negative_control_reachability.py`, issue #182) is
  the first whose subject is a **tool** rather than a record: it derives, from
  two fields of the committed `klt yield` report, whether that tool could ever
  report this campaign's negative control `detected`, and commits eleven
  executed `klt yield` probes over deliberately **synthetic** populations so the
  detection rule is checkable rather than quoted. Those probes are labelled in
  their own provenance blocks as evidence about `klt yield` and not about the
  PLL, they introduce no figure about the DUT, and the document's verdict is
  derived from the committed report alone — so the rule holds: nothing under
  `analysis/` simulates, and nothing there states a measured number the
  evidence trail does not already carry. It also derives the *forward*
  question — what population would make the control reportable, and what it
  costs — which is arithmetic over populations nobody has run, and is therefore
  the one place an `analysis/` script states a figure no record carries. The
  rule it keeps instead is that the arithmetic must reproduce the tool's own
  answers on every population that **was** run: interval bounds,
  `negative_control.verdict`, `required_n` and `sample_size.verdict` are
  re-derived for all eleven committed probes before a word of the document
  renders, and a single disagreement is a hard failure.
...
- **`<experiment-slug>`** — short, descriptive, kebab-case name for what is
  ...
  | `jitter-calibration` | the **transfer** of a known period-jitter figure through the same reducer — the sibling of `jitter-floor` one step on, and a *known-bad* control rather than a null one. Same shape of DUT (an ideal source and a load resistor, no sky130 device), same `linearize` → `wrdata` → `edge_times` → `period_jitter` path, same 200 ps grid; the difference is that this source's rising-edge schedule is drawn from a seeded RNG at an **exactly specified nonzero** RMS period jitter and written edge by edge into the netlist, so the injected figure is a re-derivable property of the committed evidence rather than a claim about a generator. Reported-against-injected over the family is a **calibration curve**: a measured figure can be read back toward a true one, where a floor only bounds it from below. Run as 2 nominal periods × 3 injected values (0.5 %, 1.0 %, 2.0 % RMS) × 3 transition times (20 ps / 200 ps / 1 ns, the same edge family `jitter-floor` uses), 300 cycles each. The two period families are named by their first letter and are both periods `jitter-floor` itself ran at: **J** = 3.9170 ns (`frac(period/step)` 0.585, trial 5's period, #185) and **K** = 3.9952 ns (`frac` 0.976, trial 2's period, #197), deliberately at opposite ends of the grid-phase range because the floor this control measures is period-specific. **Run and recorded**, 1/1 PASS each: `20260925-103212-518b31f` (J05a), `20260925-103238-518b31f` (J05b), `20260925-103303-518b31f` (J05c), `20260925-103326-518b31f` (J10a), `20260925-103348-518b31f` (J10b), `20260925-103409-518b31f` (J10c), `20260925-103435-518b31f` (J20a), `20260925-103459-518b31f` (J20b), `20260925-103526-518b31f` (J20c), `20260925-160214-ed585e4` (K05a), `20260925-160252-ed585e4` (K05b), `20260925-160316-ed585e4` (K05c), `20260925-160342-ed585e4` (K10a), `20260925-160403-ed585e4` (K10b), `20260925-160426-ed585e4` (K10c), `20260925-160453-ed585e4` (K20a), `20260925-160517-ed585e4` (K20b), `20260925-160541-ed585e4` (K20c). The headline: at an edge the grid resolves the reducer returns the injected figure **exactly** (0.500 / 1.000 / 2.000 %), and at one it cannot the reported figure is inflated 1.4×–4.8×. Read with `sim/jitter-calibration/analysis/calibration.md`, which tabulates the curve and shows the floor a *jittering* signal carries moving away from the constant-period walk-phase figure `jitter-floor` measures and toward the uniformly-spread-phase one as the source's jitter grows — i.e. the null control's floor is a different quantity from this one, not a conservative version of it — and, across the two period families, that the *sign* of the difference flips: the null control over-states that floor at `frac(period/step)` 0.585 (over-correcting) and under-states it at 0.976 (under-correcting), which #185 could only argue from the walk-phase formula and #197 measured | #185, #197 |
  ...
  | `integrator-floor` | the **integrator-error floor** under a free-running oscillator's period-jitter figure — a harness *null* control like `jitter-floor`, but with `design/vco/vco_ring5.sch`'s real ring as the DUT instead of an ideal source, because a ring's edges are placed by ngspice's timestep controller rather than by source breakpoints, and that controller's truncation error is exactly what an ideal source cannot show. Open loop at an ideal 0.86 V control voltage and ideal supply, `tt`/125 °C/1.80 V (`pll-lock-mc`'s base point, ~250 MHz), so the ring's true period jitter is zero by construction; reduced by the identical `linearize` → `wrdata` → `edge_times` → `period_jitter` path. Run as a family of variants, each record naming its own: **D** = ngspice defaults, 200 ps grid (the settings `pll-lock-mc`'s first record ran under) → **1.543 %** (and 243.8 MHz against the tighter runs' 251.1 MHz); **T** = `.options reltol=1e-4`, 200 ps grid → **0.658 %** (the residual is `jitter-floor`'s grid floor for this ~74 ps edge); **G** = `reltol=1e-4`, 20 ps grid, TMAX held at 200 ps → **0.005 %**. **Run and recorded**, 1/1 PASS each (`gate_on_bound` false — a null control cannot fail for reporting its finding): `20260925-201301-d8b1ab7` (D), `20260925-201647-d8b1ab7` (T), `20260925-203420-d8b1ab7` (G) | #202 |
  | `lf-c2-jitter-sensitivity` | how much of the closed-loop PLL's post-lock period jitter is set by **one loop-filter component** — `design/loop-filter`'s `C2`, the shunt capacitor at the charge-pump node whose stated design purpose is to attenuate reference-frequency ripple reaching `VCTRL`. Same DUT, sampling point, seed, reducer, dump grid and `reltol` as `pll-lock-mc`; a family of arms differing **only** in `C2`'s drawn area, every arm's schematic generated from the committed design files with a `--check` that fails on drift. Deliberately **not** `pll-lock-mc`'s window: the loop filter is pre-charged to 0.865 V and the window is 20 µs, so a sensitivity family is affordable — which is why it runs its own `nominal` arm and is read as a *ratio*, not as absolute figures borrowed across two conditions. No `spec_rows`: it is a sizing measurement for #195's negative control, not row 9's evidence. **Run and recorded**, 1/1 PASS each: `20260926-084846-e572408` (`nominal`, `C2` = 10.47 pF, **0.523 %** RMS), `20260926-084414-e572408` (`c2div4`, 2.64 pF, **0.551 %**), `20260926-084935-e572408` (`c2div9`, 1.19 pF, **0.566 %**). **The finding is negative and that is its value**: an 8.8× reduction in the ripple-rejection capacitor moves the measured figure by 8 % of itself, with `f_out`, duty and time-to-lock unchanged — so `C2` is not a lever on row 9's quantity at this operating point, and it is rejected as the negative control's knob by measurement rather than by opinion. The `nominal` arm also validates the method: 0.523 % against `pll-lock-mc`'s own 0.554 % at the same seed, 5.6 % apart | #195 |
  | `lf-c1-jitter-sensitivity` | sibling of `lf-c2-jitter-sensitivity` — byte-identical `measure` block, same draw, same window and initial conditions, **different degraded component**: `design/loop-filter`'s `C1`, the compensation-zero capacitor that per `design/loop-filter/DESIGN.md`'s own derivation sets *both* the loop crossover (row 6) and the phase margin (row 7), so shrinking it leaves a badly under-damped loop. It exists because its sibling's `C2` result came back negative. Has **no nominal arm of its own** — it cites the sibling's, and `sim/tests/test_negative_control_variant.py` asserts the two manifests still agree so that citation cannot go stale. **Run and recorded**, 1/1 PASS each: `20260926-110626-044c894` (`c1div9`, `C1` = 23.19 pF, **0.983 %** RMS, locked 347.4 ns), `20260926-130047-c2ecc77` (`c1div12`, 17.41 pF, **1.274 %**, locked 1.531 µs), `20260926-130144-c2ecc77` (`c1div16`, 13.07 pF, **3.086 %**, locked 13.13 µs), `20260926-110714-044c894` (`c1div25`, 8.39 pF, **no lock within 20 µs** — and therefore no jitter figure at all, row 9's population being post-lock). `C1` moves this figure by **490 % over a 16× change** where `C2` moved it by 8 % over 8.8×, and past area/16 the loop stops locking — so at this operating point the measured period jitter tracks the loop's **damping**, not the filter's ripple rejection. That bracket is the sizing answer #195 needed: `c1div12` is the arm that both misses the ratified bound (by 27 %) and locks with margin, and it is `pll-lock-mc-negative-control`'s DUT | #195 |
  | `pll-lock-mc-negative-control` | **T1 item 6's deterministic negative control** for `pll-lock-mc`'s campaign: the same testbench, sampling point, reducer, window, lock criterion and ratified 1.0 % bound, drawn against a **deliberately degraded variant of the DUT** (`design/loop-filter`'s `C1` at a twelfth of its drawn area, sized by `lf-c1-jitter-sensitivity`). Unlike `jitter-floor` (a *null* control, #178) and `jitter-calibration` (a known-bad *measurement input*, #185), this one degrades the **design** — which is the kind `klt yield`'s `negative_control` and item 6's checklist ask for, and the reason neither of the other two can be declared as this campaign's control (`signoff/README.md` § "Why every other row is `unmet`" case 4b). Six draws, because per `analysis/negative-control/reachability.md` an all-failing control separates from the now 5-of-5 nominal population at six (Clopper-Pearson upper bound 0.459258 vs the nominal's lower bound 0.478176) and not at five — and only if every draw both locks *and* misses, since a no-lock draw is censored out of the control's own denominator. **Testbench only, no record yet**: the campaign is six 50 µs cold-start transients at `reltol=1e-4`, and no host is provisioned for the batch/remote fleet (`sim/executor-equivalence/README.md`, 2AMLogic/2am#934) | #195 |
...
- **The dump grid was not the only floor: ngspice's integrator is a second,
  and at this campaign's first settings it alone exceeds the bound**
  (`sim/integrator-floor`, issue #202). Every null control above has an ideal
  *source* as its DUT, whose edges sit on the source's own breakpoints; a
  free-running ring's edges are placed by the timestep controller instead, so
  its truncation error is a floor those controls cannot show. Run on
  `design/vco`'s own ring, open loop at this campaign's operating point (true
  period jitter zero by construction), the reducer reports **1.543 %** at the
  settings `20260924-222341-a9375a5` ran under (ngspice defaults, 200 ps grid),
  **0.658 %** with `.options reltol=1e-4` alone, and **0.005 %** with that plus
  a 20 ps dump grid (TMAX held at 200 ps, so the integration is unchanged). The
  first figure is larger than row 9's whole budget for a DUT that has no
  jitter at all, so the first record's three misses cannot be read as design
  misses, and the jitter-floor restatement above — which removes only the
  grid's share — does not settle them either. The manifest now declares
  ...
  `sim/pll-lock-mc/records/20260925-224917-3a2dd6e.md`, **5/5 draws lock and
