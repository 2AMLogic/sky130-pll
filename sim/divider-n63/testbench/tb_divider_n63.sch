...
* tb_divider_n63.sch -- standalone divider_intN PVT testbench, N=63 (issue #129)
*
...
* comment for the full rationale. This testbench straps N=63
* (NSEL[5:0]=111110), the largest *odd* modulus in the DRAFT range
...
* table) combined with the widest reload word (`L`=30), together the
* largest-depth odd-path case the decode reaches. CLK is fixed at 250 MHz,
* the design's own lock target. Per issue #129's scope, this record covers
* only the two named corners (tt/27 degC/1.80 V and ss/125 degC/1.62 V)
* rather than the full grid -- see this directory's tb.json
* `methodology_note` and the record's own subset-reason field.
*
...
"
spice_ignore=false}
C {devices/title.sym} -200 500 0 0 {name=l4 author="2AM Logic (issue #129, sky130 standalone divider_intN PVT testbench, N=63)"}
