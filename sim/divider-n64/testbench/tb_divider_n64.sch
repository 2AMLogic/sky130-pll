...
* tb_divider_n64.sch -- standalone divider_intN PVT testbench, N=64 (issue #129)
*
...
* comment for the full rationale. This testbench straps N=64
* (NSEL[5:0]=111111), the DRAFT modulus range's ceiling (spec/target-spec.md
* row 4, N = 4-64) and the largest modulus that takes the divider's *even*
* decode path (ODD=0; design/divider/DESIGN.md's "Modulus correctness
* across the DRAFT range" table) -- it also exercises the widest reload
* word (`L`=31) and the static NSEL-1 decode's own borrow chain running its
* full width. CLK is fixed at 250 MHz, the design's own lock target.
*
...
C {devices/lab_pin.sym} -130 -60 0 0 {name=p5 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} -130 -20 0 0 {name=p6 sig_type=std_logic lab=VDD}
...
C {devices/lab_pin.sym} -130 140 0 0 {name=p10 sig_type=std_logic lab=VDD}
...
"
spice_ignore=false}
C {devices/title.sym} -200 500 0 0 {name=l4 author="2AM Logic (issue #129, sky130 standalone divider_intN PVT testbench, N=64)"}
