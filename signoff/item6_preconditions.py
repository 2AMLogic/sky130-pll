    ...
    return [
        ...
        },
        {
            ...
            "closes": (
                ...
                if control_verdict == "detected"
                    ...
                    "measurable draws with none missing the bound, against a 2-draw "
                    "control, and a campaign passing half the time costs ~50x that. "
                    f"The derivation, checked against eleven executed `klt yield` "
                    f"probes, is in `{REACHABILITY}`. No amount of simulator time "
                    "spent on the control alone moves this row"
        ...
        },
        {
            ...
            "closes": (
                ...
                "against the "
                f"{facts['sample_size_n']} this one has -- but read that figure "
                "with its branch attached, because it is not a fixed target. "
                f"`klt yield` reports `required_n` = {facts['required_n']} here "
                "from its exact zero-failures method, i.e. for a campaign in "
                "which **no** draw passes; at an intermediate pass rate it "
                "switches to a normal approximation whose `required_n` peaks in "
                "the middle (1797 measurable draws at 9 of 183 passing, measured). "
                "So this row and row 3 are jointly cheapest at the two ends and "
                f"dearest in between -- see `{REACHABILITY}` for the derivation and "
                "`sim/pll-lock-mc/analysis/README.md` § \"The sample-size "
                "question, answered\" for why that spend is not yet the right one"
            ),
        },
    ]
